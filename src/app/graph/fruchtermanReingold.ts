...
export function fruchtermanReingold3(
  ...
  for (const regionA of nodes) {
    for (const regionB of nodes) {
      if (regionA.index === regionB.index) continue;
      const nodeA = nodes[regionA.index];
      const nodeB = nodes[regionB.index];
      if (!nodeA || !nodeB || !adjacencyMatrix[regionA.index]) continue;
      const dx = nodeB.x - nodeA.x;
      const dy = nodeB.y - nodeA.y;
      const distance = Math.sqrt(dx * dx + dy * dy) || 1;
      const adjDistance = adjacencyMatrix[regionA.index][regionB.index];
